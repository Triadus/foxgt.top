...
if ($cfg['plugin']['tagslance']['projects'])
{
	...
	while ($tc_row = $tc_res->fetch())
	{
		...
		$tc_html .= cot_rc('tags_link_cloud_tag', array(
			'url' => cot_url('plug', array('e' => 'tags', 'a' => 'projects', 't' => str_replace(' ', '-', $tag_u), 'tl' => $tl)),
	...
	{
		$t->assign('PRJ_TAG_CLOUD_ALL_LINK',
			cot_rc('tags_code_cloud_more', array('url' => cot_url('plug', 'e=tags&a=projects'))));
	}
}
